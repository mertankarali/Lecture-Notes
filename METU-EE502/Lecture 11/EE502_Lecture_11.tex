...
\end{align*}
%
Moreover, by the Cayley-Hamilton theorem, we can also conclude that $C A^{l} x_u = 0, \ \forall l \in \mathbb{Z}^{\geq 0}$. Now let's analyze the zero-input response of the system with $x(0) = x_u$:
%
\begin{align*}
...
x_u \in \mathcal{N}[ \mathbf{O} ]$. If $x_u \in \bar{\mathcal{O}}_T$, then the zero-input output response is
%
\begin{align*}
  y_{zi}(t) = C e^{A t} x_u = 0 , \, \forall t \in [0,T] \, 
  \Rightarrow \, \begin{array}{c} 
  y_{zi}(0) = 0 \, \Rightarrow \, C x_u = 0
  \\
  \left[\frac{d}{dt}y_{zi}(t) \right]_{t=0} = 0 \, \Rightarrow \, C A x_u = 0
  \\
  \left[\frac{d^2}{dt^2}y_{zi}(t) \right]_{t=0} = 0 \, \Rightarrow \, C A^2 x_u = 0
  \\
  \vdots
  \\
  \left[\frac{d^{n-1}}{dt^{n-1}}y_{zi}(t) \right]_{t=0} = 0 \, \Rightarrow \, C A^{n-1} x_u = 0
  \end{array}
...
For a CT-LTI system, the observability Grammian is defined as
%
\begin{align*}
...
If $x_u \in \mathcal{N}[ \mathbf{O} ]$, then we know that $C A^{l} x_u = 0, \ \forall l \in \mathbb{Z}^{\geq 0}$. Let's analyze 
...
\begin{align*}
    \textbf{Q}(t) x_u = 0 \, \Rightarrow \, x_u^T \textbf{Q}(t) x_u = 0 \, \iff \, \int\limits_{0}^{t} x_u^T e^{A^T (t - \tau)} C^T C e^{A (t - \tau)} x_u d \tau
...
\end{align*}
%
Note that since the unobservable sub-space is $A$ invariant, $A T_{\bar{o}}$ must remain in $\mathrm{Ra}(T_{\bar{o}}) = \mathcal{N}(\mathbf{O}) = \bar{\mathcal{O}}$. 
Since the columns of $T_2$ are linearly independent from $T_{\bar{o}}$, for $T_2 A_{12} + T_{\bar{o}} A_{22}$ to remain in $\mathrm{Ra}(T_{\bar{o}})$, we must have $A_{12} = \mathbf{0}$.
...
\\
y &= \left[ \begin{array}{c|c} C_1 & 0 \end{array} \right] \bar{x} + D u
\end{align*}
%
...
\begin{align*}
    \dot{x} &= \begin{bmatrix} -1 & 0 & 0 \\ 1 & -2 & 1 \\ 0 & 0 & -3 \end{bmatrix} x \quad , \quad y = \begin{bmatrix} 1 & 0 & 0 \end{bmatrix} x
\end{align*}
...
\textbf{Solution:} \\
\textbf{(a) Unobservable Subspace:} By partitioning, we have the observable part $A_{11} = -1$ and the unobservable block $A_{22} = \begin{bmatrix} -2 & 1 \\ 0 & -3 \end{bmatrix}$. The observability matrix is:
\begin{align*}
...
\textbf{(b) Unobservable Modes:} The unobservable block $A_{22}$ has eigenvalues $\lambda_2 = -2$ and $\lambda_3 = -3$. The corresponding right eigenvectors for the full matrix $A$ are $v_2 = \begin{bmatrix} 0 & 1 & 0 \end{bmatrix}^T$ and $v_3 = \begin{bmatrix} 0 & 1 & -1 \end{bmatrix}^T$. Because $v_2, v_3 \in \mathcal{N}[\mathbf{O}]$, we get $C v_2 = 0$ and $C v_3 = 0$. The modal responses $e^{-2t}$ and $e^{-3t}$ are completely unobservable. 

\textbf{(c) Zero-Output Demonstration:} If we start the system with an initial condition strictly in the unobservable subspace, say $x(0) = \begin{bmatrix} 0 \\ 0 \\ 1 \end{bmatrix}$ (which can be formed by $v_2 - v_3$), the internal state evolution is:
\begin{align*}
    x(t) = e^{At} x(0) = e^{At} (v_2 - v_3) = e^{-2t} v_2 - e^{-3t} v_3 = \begin{bmatrix} 0 \\ e^{-2t} - e^{-3t} \\ e^{-3t} \end{bmatrix}
\end{align*}
However, the measured zero-input output remains identically zero:
\begin{align*}
    y(t) = C x(t) = \begin{bmatrix} 1 & 0 & 0 \end{bmatrix} \begin{bmatrix} 0 \\ e^{-2t} - e^{-3t} \\ e^{-3t} \end{bmatrix} = 0 \quad \text{for all } t \ge 0
\end{align*}
...
\begin{itemize}
 ...
 \item $C v \neq 0 \, , \, \forall v \in \lbrace v | v \neq 0 \, , \, A v = \lambda v , \, \lambda \in \mathbb{C} \rbrace$ $\iff$ 
 \item $\mathrm{rank} \left[ \begin{array}{c} \lambda I - A \\ \hline C \end{array} \right] = n , \ \forall \lambda \in \mathbb{C}$
\end{itemize}
