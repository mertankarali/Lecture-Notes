...
\end{exmp}
\begin{itemize}
    \item Show that $\mathcal{R} \subset \matjcal{C}$, $\forall (A,B)$, however $\mathcal{C} \subset \matjcal{R}$ not necessarily true $\forall (A,B)$.
    \item Similar to the reachable subspace, characterize the controllable subspace
    \item Derive conditions such that $\mathcal{R} = \matjcal{C}$
\end{itemize}

\subsubsection{Reachability Gramian}
...
If $x \in \mathrm{Ra} (\mathbf{P}_m)$, then $\exits v \in \mathbb{R}^n$ s.t. $x = \mathbf{P}_m v$ then
%
\begin{align*}
