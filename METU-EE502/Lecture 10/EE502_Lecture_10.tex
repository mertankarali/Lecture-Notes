...
\newtcolorbox{intuition}{colback=blue!5!white,colframe=blue!75!black,title=Intuition \& Insight, arc=1mm, fonttitle=\bfseries}
\newtcolorbox{takeaway}{colback=red!5!white,colframe=red!75!black,title=Key Takeaway, arc=1mm, fonttitle=\bfseries}
...
For a discrete-time LTI state-space representation
%
\begin{align*}
  x[k+1] &= A x[k] + B u[k]
\\
...
%
\begin{itemize}
  \item A state $x_r$ is said to be $m$-step \textcolor{blue}{\textbf{reachable}}, if there exist
  ...
  \item A state $x_d$ is said to be $m$-step \textcolor{red}{\textbf{controllable}},
...
\end{align*}
%
then if a state $x_r$ is reachable at $m$ steps, it should 
satisfy the following equation for some $\mathbf{U}_m$.
%
\begin{align*}
 \mathbf{R}_m \mathbf{U}_m &= x_r
\end{align*}
%
In order for this matrix equation to have a solution, $x_r$
should be in the range space of $\mathbf{R}_m$.
%
\begin{align*}
  x_r \in \mathrm{Ra} ( \mathbf{R}_m ) 
\end{align*}
%
Thus the $m$-step reachable sub-space is simply equal to the range space of $\mathbf{R}_m$
\begin{align*}
...
\end{align*}
%
In order to prove $\mathcal{R}_{n} = \mathcal{R}_{l} $, we simply use the Cayley-Hamilton theorem. 
...
%
\begin{align}
 \mathbf{P}_m &= \mathbf{R}_m {\mathbf{R}_m}^T =\sum\limits_{i=0}^{m-1} A^i B B^T \begin{pmatrix} A^T \end{pmatrix}^i
\end{align}
%
Note that $\mathbf{P}_m$ is a symmetric positive semi-definite matrix.

\textbf{Lemma:} $\mathcal{R}_m = \mathrm{Ra}( \mathbf{R}_m ) = \mathrm{Ra} ( \mathbf{P}_m )$

\textbf{Proof:} Let's first show that $\mathrm{Ra}( \mathbf{P}_m ) \subset \mathrm{Ra}( \mathbf{R}_m )$. 
If $x \in \mathrm{Ra} (\mathbf{P}_m)$, then $\exists v \in \mathbb{R}^n$ s.t. $x = \mathbf{P}_m v$ then
%
\begin{align*}
 x = \mathbf{P}_m v &= \mathbf{R}_m {\mathbf{R}_m}^T v = \mathbf{R}_m y \ ( \text{where } y = {\mathbf{R}_m}^T v ) \ \Rightarrow \ x \in \mathrm{Ra}( \mathbf{R}_m ) \ \Rightarrow \ \mathrm{Ra}( \mathbf{P}_m ) \subset \mathrm{Ra}( \mathbf{R}_m )
\end{align*}
...
\end{align*}
%
This completes the proof. As a result of this lemma, full reachable subspace $\mathcal{R} = \mathrm{Ra}( \mathbf{P}_l )$ for any $l \geq n$.
...
\begin{align*}
 ...
 \\
 A x &= \left[ \begin{array}{c|c|c|c|c} \sum\limits_{i=0}^{n-1} -a_i A^{i} B & A^{n-2} B &
         \cdots & A^2 B & A B \end{array} \right] \textbf{U}_n 
 \\
...
\end{align*}
%
Now let's show that if $(A,B)$ is not reachable, then $\exists w$ s.t. $w^T A = \lambda w^T$ and $w^T B = 0$.  If $(A,B)$ is not reachable, then there exists $q \in \mathbb{R}^n$ such that
%
\begin{align*}
...
\end{align*}
%
Let $\nu$ be an eigenvector of $\Gamma$, i.e. $\Gamma \nu = \alpha \nu , \, \alpha \in \mathbb{C}$, then 
%
\begin{align*}
A^T V \nu = V \Gamma \nu = \alpha V \nu \ \rightarrow \ z = V \nu \in \mathbb{C}^n
\end{align*}
...
\textbf{Theorem:} \textit{PBH Reachability Test} - $(A,B)$ is reachable if and only if 
%
\begin{align*}
    \mathrm{rank} \left[ \begin{array}{c|c} \lambda I - A & B \end{array} \right] = n , \ \forall \lambda \in \mathbb{C}
\end{align*}
%
...
\begin{intuition}
\textbf{Physical Meaning of Modal Tests:} 
A left eigenvector $w^T$ corresponds to a specific "mode" (dynamic behavior or natural frequency) of the system. The matrix product $w^T B$ represents how the input $u$ "couples" into or excites that specific mode. 

If $w^T B = 0$, the input has absolutely no channel to influence that mode. The mode will simply evolve on its own, rendering those states out of our control (unreachable).
\end{intuition}
...
to highlight reachable and unreachable ``spaces''. Let 
%
\begin{align*}
...
since $T_2$ is composed of linearly independent columns, in order for $T_1 A_{11} + T_2 A_{21}$ to remain in $\mathrm{Ra}(T_1)$, $A_{21} = \mathbf{0}$.
...
Note that
\begin{itemize}
  \item the set $\mathcal{R}_{t_f}$ of all states that are reachable in time $t_f$ is a linear
(sub)space: $\mathcal{R}_{t_f} \subset \mathbb{R}^n$
  \item the set $\mathcal{C}_{t_f}$ of all states that are controllable in time $t_f$ is a linear
(sub)space: $\mathcal{C}_{t_f} \subset \mathbb{R}^n$
\end{itemize}

Let's characterize $\mathcal{R}_{t_f}$. In CT, the derivation of the reachability 
...
\end{align*}
%
Obviously if an $x_r \in \mathbb{R}^n$ is reachable in time $t_f$, 
...
\begin{intuition}
\textbf{Why use the Gramian?} 
While the Reachability Matrix $\mathbf{R}$ tells us \textit{if} a system is reachable (a binary yes/no), the Reachability Gramian $\textbf{P}(t_f)$ provides a quantitative measure of \textit{how} reachable the states are. The eigenvectors of $\textbf{P}(t_f)$ indicate the easiest/hardest spatial directions to reach, and its eigenvalues relate to the control energy $\int ||u(t)||^2 dt$ required to get there.
\end{intuition}
...
\end{align}
%
Let $u(t) = B^T e^{A^T (t_f - t)} v$, then
%
\begin{align*}
  x(t_f) &= \int\limits_{0}^{t_f} e^{A (t_f - \tau)} B u(\tau) d \tau
  \\
...
\end{align*}
%
Let's first show that $\mathrm{Ra}^\perp [ \mathbf{R} ] \subset \mathrm{Ra}^\perp [ \textbf{P}(t_f) ]$.
Let $q \in \mathrm{Ra}^\perp [ \mathbf{R} ] $, then 
%
\begin{align*}
  q^T \mathbf{R} = 0 \, &\iff \, q^T \left[ \begin{array}{c|c|c|c|c} A^{n-1} B & A^{n-2} B & \cdots & A B & B \end{array} \right] = 0 \ \iff \, q^T A^i B = 0 \, \forall i \ge 0
\end{align*}
%
Note that for $i \geq n$, we referred to the Cayley-Hamilton theorem. Now let's check if 
$q \in \mathrm{Ra}^\perp [ \mathbf{P}(t_f) ] $
%
\begin{align*}
  q^T \mathbf{P}(t_f) &= \int\limits_{0}^{t_f} q^T e^{A (t_f - \tau)} B B^T e^{A^T (t_f - \tau)}  d \tau 
  \\
  q^T A^i B &= 0 \, \forall i \ge 0 \, \Rightarrow \, q^T e^{A (t_f - \tau)} = 0 \, \forall t_f \geq 0
  \\
  &\Rightarrow \, q^T \mathbf{P}(t_f) = 0 \, \Rightarrow \, q \in \mathrm{Ra}^\perp [ \mathbf{P}(t_f) ]
...
%
\begin{align*}
  q^T e^{A (t_f - \tau)} B = 0 \, \forall \, \tau \in [0 , t_f]
\end{align*}
%
Then we know that 
%
\begin{align*}
  [ q^T e^{A \eta} B]_{\eta = 0} = 0 \, &\Rightarrow \, q^T B = 0
  \\
  \frac{d}{d\eta}[ q^T e^{A \eta} B]_{\eta = 0} = 0 \, &\Rightarrow \, q^T A B = 0
  \\
  \frac{d^2}{d\eta^2}[ q^T e^{A \eta} B]_{\eta = 0} = 0 \, &\Rightarrow \, q^T A^2 B = 0
  \\
  &\vdots
    \\
    \frac{d^{n-1}}{d\eta^{n-1}}[ q^T e^{A \eta} B]_{\eta = 0} = 0 \, &\Rightarrow \, q^T A^{n-1} B = 0
    \\
...
reachability matrix, $\mathbf{R}$, characterizes the whole reachable space, and this space is independent
...
\begin{align*}
  ...
  \lambda_1 - \lambda & & 0 & b_1 \\
    & \ddots &
    \\
    0 & &  \lambda_n - \lambda & b_n \end{array} \right]
\end{align*}
%
...
\textit{Sufficient Condition:} Note that if $\lambda_i \neq \lambda_j$ for $i \neq j$,
...
is a linearly independent set. We can see that the number of inputs of the system has to be larger than 
or equal to $p$ (indeed $\mathrm{max}(p)$ for system-wide reachability). Moreover, the rank of the 
following sub-matrix has to be equal to the algebraic multiplicity of the eigenvalue, i.e.
%
\begin{align*}
  B_{\lambda_1}
  &= \left[ \begin{array}{c} 
...
\textbf{Solution:} Let's first assume that $\lambda_1 \neq \lambda_2$, we can analyze each mode separately using the PBH test. Let's start with $\lambda = \lambda_1$
%
\begin{align*}
  \left[ \begin{array}{c|c} A - \lambda_1 I & B \end{array} \right]
  &= \left[  \begin{array}{ccc|cc|c} 0 & 1 & 0 & 0 & 0 & b_{1,1} \\
    ...
    0 & 0 & 0 & 0 & \lambda_2 - \lambda_1 & b_{2,2}   \\
    \end{array} \right]
\end{align*}
%
...
\end{align*}
%
Since $B$ is a single column vector (single-input system), it is impossible for its components to span $\mathbb{R}^2$. Therefore, the matrix drops rank, and the system is strictly unreachable when $\lambda_1 = \lambda_2$.
