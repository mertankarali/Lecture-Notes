...
%
\begin{itemize}
  \item A state $x_r$ is said to be \textbf{reachable} in time $t_f > 0$, if there exist
...
for an CT-LTI system $x(t_f)$ for a zero-state response written as
%
\begin{align*}
...
Let $x^* \in \mathrm{Ra}[ \textbf{P}(t_f) ]$, then $\exist v \in \mathbb{R}^n$ such that 
%
\begin{align}
    x^* = \textbf{P}(t_f) v
\end{align}
