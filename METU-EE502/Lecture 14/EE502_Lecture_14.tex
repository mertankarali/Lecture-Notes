...
% Custom command to highlight edits in blue
...
Generally the full state \edit{measurement}, $x(t)$ or $x[k]$, of a system is not accessible and \edit{(observers, estimators, filters)}
...
controller, we assume that it is known a priori. If $x[0] = \hat{x}[0]$ and
when there is no model mismatch or uncertainty in the system, then
we expect that $x[k] = \hat{x}[k]$ and $y[k] = \hat{y}[k]$ 
for all $k$. When $x[0] \neq \hat{x}[0]$, then we should observe a 
difference between the measured and predicted output
$y[k] \neq \hat{y}[k]$. The core idea in a Luenberger observer
...
$A v_u = v_u \lambda_u $ and $C v_u = 0$. Now check whether $v_u$ is a right eigenvector of $(A - L C)$
%
\begin{align*}
