...
\begin{itemize}
  ...
  \item A state $x_d$ is said to be \textbf{controllable}
  if there exist a finite time interval $t \in [0 ,t_f]$ and
  an input signal defined on this interval, $u(t)$, that transfers the state vector 
...
Unlike CT systems the Reachability and Controllability conditions 
are not equivalent. 
...
necessarily imply Reachability. For this reason, the term of Reachability 
...
\end{align*}
%
Thus input has no effect on the states. If $x[0] = 0$, then
...
to guarantee whether $x_0$ is unobservable or not. Formally speaking,
%
\begin{align*}
...
$\mathrm{rank}( \mathbf{O} ) = 1$, thus the state-space representation
...
\begin{align*}
\mathrm{det} \left( \left[ \begin{array}{cc} \lambda & -1 \\ -1 & \lambda \end{array} \right] \right)
 &= \lambda^2 - 1
...
\left[ \begin{array}{cc} z & -1 \\ -1 & z\end{array} \right]^{-1}
\left[ \begin{array}{c} 0 \\ 1 \end{array} \right]
\\
&=
\left[ \begin{array}{cc} 1 & -1 \end{array} \right]
\left[ \begin{array}{cc} z & 1 \\ 1 & z\end{array} \right]
...
with $\lambda = 1$ is reachable but not observable. For this
