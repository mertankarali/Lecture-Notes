...
\begin{enumerate}
  ...
  \item The Analog-to-Digital Converter (ADC)
  \item The Controller ($\mu P$), a microprocessor/microcontroller
    with a ``real-time'' OS 
 \item The Digital-to-Analog Converter (DAC) 
\end{enumerate}
...
system. In this course our focus is LTI systems thus, we will assume
that (unless otherwise is given) that the plant is a continuous LTI
plant model with a transfer function of $G_c(s)$ for which both the
input and output are continuous time signals. 

The ``digital'' blocks of the closed-loop block diagram structure are 
ADC, The Controller, and DAC. It is generally assumed (design
preference) that all blocks shares a common ``hard real time'' clock.
...
is a discrete-time signal, $e[k]$, where the relating between the
