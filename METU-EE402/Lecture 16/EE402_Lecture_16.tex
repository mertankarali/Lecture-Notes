...
system (e.g. $y[k] = x[k]$) are available, the most
popular control method is the linear state feedback
control,
%
\begin{align*}
 u[k] = - K x[k]
...
$\hat{G} = G - H K$. Important questions is how to choose $K$. 
Note that 
%
\begin{align*}
...
\end{align*}
%
As in all of the control desgn techniques, the most critical criterion
is stability, thus we want all of the eigenvalues to be strictly
inside the unit-circle. However, we know that there could be 
...
is that the system should be Reachable.
