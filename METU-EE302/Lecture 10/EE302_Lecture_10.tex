...
In this lecture we will analyze the effects of PID coefficients on 
...
\end{align*}
% 
Thus steady-state error for unit-step and unit-ramp inputs can be found
as
\begin{itemize}
...
1$), them increasing the P gains has a negative effect on 
...
\end{align*}
%
Obviously, steady-state performance is better with $K_p = 5$
...
\end{align*}
%
Obviously, steady-state performance is better with $K_p = 8$
...
We verify that steady-state performance is better with a larger
...
\end{align*}
%
Let's first analyze the effect of $K_D$ term on steady-state 
...
form. One should note the fact that, the existance of closed-loop
...
the $K_D s = 4 s$ term in the numerator effects the output behavior and
its effect should be maximum when $0 < t < t_s$. In order see how
...
In this context, we should think of the transient performance
...
The gap between settling time estimates come form the effect
...
effect of closed-loop zeros on the output behavior. 
